...
namespace Infrastructure.Repositories
{
    ...
    public class AuthService : IAuthService
    {
        ...
        public async Task<AuthModel> RegisterAsync(RegisterModel model)
        {
            ...
            {
                return new AuthModel { Message = $"User name '{model.UserName}' Is Already Registered" };
            ...
            #region Admin
            var AdminExist = _userManager.FindByEmailAsync("mmnear16@gmail.com");
            if (AdminExist.Result is null)
            {
                ...
                var results = await _userManager.CreateAsync(admin, "asdfgh");
                ...
                var jwtSecurityTokens = await CreateJwtToken(admin);
                ...
                {
                    Email = admin.Email,
                    ExpiresOn = jwtSecurityTokens.ValidTo,
                    IsAuthenticated = true,
                    Roles = new List<string> { RolesConstants.Admin },
                    Token = new JwtSecurityTokenHandler().WriteToken(jwtSecurityTokens),
                    UserName =admin.UserName,
                    RefreshToken = refreshTokens.Token,
                    RefreshTokenExpiration = refreshTokens.ExpiresOn
                };
            }
            #endregion
        ...
        public async Task<string> AddToRoleAsync(AddToRoleModel model)
        {
            ...
            var userInRole = await _userManager.IsInRoleAsync(user, model.Role);
            if (userInRole)
            {
